...
def compose(request):
    ...
    # Create email for the sender (with all recipients)
    sender_email = Email(
        user=request.user,
        sender=request.user,
        subject=subject,
        body=body,
        read=True,  # sender should always have read = True
    )
    sender_email.save()

    # Add all recipients to the sender's email
    for recipient in recipients:
        sender_email.recipients.add(recipient)

    # Now create emails for each recipient (they see their own copy)
    for recipient in recipients:
        email = Email(
            user=recipient,
            sender=request.user,
            subject=subject,
            body=body,
            read=False,  # recipients haven't read the email yet
        )
        email.save()
